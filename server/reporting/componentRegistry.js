...
function validateColumns(columns, path) {
  ...
  columns.forEach((column, index) => {
    if (!column || typeof column !== 'object' || !String(column.label || '').trim()
      ...
      || Object.keys(column).some((key) => !['label', 'key'].includes(key))) {
...
function validateFields(fields, path) {
  ...
  fields.forEach((field, index) => {
    if (!field || typeof field !== 'object' || !String(field.label || '').trim()
      || Object.keys(field).some((key) => !['label', 'binding'].includes(key))) {
      throw reportError('report_component_properties_invalid', 400, { path: `${path}.${index}` });
    }
...
function validateStyles(styles) {
  ...
  if (!styles || typeof styles !== 'object' || Array.isArray(styles)
      || Object.keys(styles).some((key) => !['page_orientation', 'font_scale', 'accent_color'].includes(key))) {
...
function validateComponentTree(input) {
  ...
  tree.components.forEach((component, index) => {
    ...
    if (component.type === 'header') allowedProperties.add('subtitle_binding');
    if (FIELD_COMPONENTS.has(component.type)) allowedProperties.add('fields');
  ...
  const sections = tree.components.map((component) => {
    ...
    if (component.type === 'header') {
      ...
      section.data = component.fields.map((field) => ({
        label: field.label,
        value: normalizeCell(atPath(context, field.binding)),
      }));
      ...
      section.columns = columns.map((column) => ({ label: column.label, key: column.key }));
      section.data = rows.map((row) => columns.map((column) => normalizeCell(atPath(row, column.key))));
      if (policyOverview) {
